  ...
  return (
    <div className="rounded-md border border-[#e5e8ee] bg-white p-5 shadow-sm">
      ...
      {field.type === 'textarea' ? (
        ...
        <CustomSelect id={id} value={value || ''} placeholder={field.placeholder || 'Selecione'} options={field.options || []} error={error} onChange={(nextValue) => onChange(key, nextValue)} />
      ) : field.type === 'single' || field.type === 'radio' ? (
        <div className="mt-3 grid gap-2">
          {(field.options || []).map((option) => <button key={option} type="button" onClick={() => onChange(key, option)} className={`cursor-pointer rounded-md border px-4 py-3 text-left text-[14px] transition hover:border-[#eb001a] ${value === option ? 'border-[#eb001a] bg-[#fff1f3] text-[#eb001a]' : 'border-[#dfe3ea] bg-white text-[#111318]'}`}>{option}</button>)}
        </div>
      ) : (
  ...
  return (
    <aside className="rounded-md bg-[#07090c] p-7 text-white shadow-[0_18px_45px_rgba(0,0,0,0.22)] lg:sticky lg:top-6">
      ...
      <nav className="mt-8 space-y-3" aria-label="Seções do diagnóstico">
        {steps.map((step, index) => <button key={step.key || index} type="button" onClick={() => index <= currentStep && onJump(index)} className={`flex w-full items-center gap-3 rounded-md px-4 py-4 text-left text-[13px] font-semibold ${index <= currentStep ? 'cursor-pointer' : 'cursor-not-allowed opacity-55'} ${index === currentStep ? 'bg-[linear-gradient(90deg,rgba(235,0,26,0.28),rgba(235,0,26,0.04))] text-[#eb001a]' : 'text-white/78 hover:bg-white/[0.04]'}`}><Users className="h-4 w-4" />{step.title}</button>)}
      </nav>
...
function DiagnosticScreen({ selectedForm, answers, currentStep, errors, submitting, onAnswer, onBack, onJump, onNext }) {
  const steps = useMemo(() => [...selectedForm.sections, contactStep], [selectedForm])
    ...
    <main className="bg-white text-[#111318]">
      ...
      <section className="mx-auto grid max-w-[1180px] grid-cols-[270px_minmax(300px,1fr)_220px] gap-8 px-4 pb-14 max-[900px]:grid-cols-1">
        ...
        <section>
          ...
          <div className="mt-8 flex gap-3 max-[520px]:flex-col"><button type="button" onClick={onBack} className="inline-flex h-12 cursor-pointer items-center justify-center gap-2 rounded-md border border-[#e0e3e8] bg-white px-5 text-[14px] font-semibold"><ArrowLeft className="h-4 w-4" />Voltar</button><button type="button" onClick={onNext} disabled={submitting} className="inline-flex h-12 flex-1 cursor-pointer items-center justify-center gap-3 rounded-md bg-[#e60018] px-6 text-[14px] font-bold text-white disabled:cursor-not-allowed disabled:opacity-60">{submitting ? 'Enviando...' : currentStep === steps.length - 1 ? 'Ver resultado' : 'Continuar'}<ArrowRight className="h-4 w-4" /></button></div>
        </section>
...
export default function AnalisePage() {
  ...
  function handleNext() {
    if (!selectedForm) return
    const steps = [...selectedForm.sections, contactStep]
    const step = steps[currentStep]
    if (!validateStep(step)) return
    if (currentStep < steps.length - 1) { setCurrentStep((current) => current + 1); return }
    submitDiagnosis()
  }

  function handleBack() {
    setErrors({})
    if (currentStep > 0) { setCurrentStep((current) => current - 1); return }
    setStage('segment')
  }
  ...
  return selectedForm ? <DiagnosticScreen selectedForm={selectedForm} answers={answers} currentStep={currentStep} errors={errors} submitting={submitting} onAnswer={handleAnswer} onBack={handleBack} onJump={setCurrentStep} onNext={handleNext} /> : null
}
